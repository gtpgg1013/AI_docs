...
########################################################################################
    ...
    net = cv2.dnn.readNetFromCaffe("MobileNetSSD_deploy.prototxt.txt", "MobileNetSSD_deploy.caffemodel") # txt와 모델 적용 (같은 디렉터리 안에 있음)
    ...
    for i in np.arange(0, detections.shape[2]):
        ...
        if confidence > CONF_VALUE: # 신뢰값 정의
...
def deep2OpenCV():
    ...
    # 여기서는 동영상 열어야함 : 게다가 tkinter는 느리니까 이거 캡쳐해서 쓰자(필요한 장면만)
    filename = askopenfilename(parent=window,
                               filetypes=(("동영상 파일","*.mp4"), ("모든 파일", "*.*")))
    if filename == '' or filename == None:
        return

    cap = cv2.VideoCapture(filename) # filename 대신 0 쓰면 자동 연결된 카메라 사용가능
    s_factor = 0.5 # 화면 크기 비율

    # 사람수 측정
    _numOfPeople = 0
    _numOfPeopleProbList = []
    _numOfSthElse = 0

    frameCount = 0
    while True:
        ret, frame = cap.read() # 현재 한 장면 캡쳐 # 여기서 읽을 frame은 이미지 객체, (y,x,RGB)
        if not ret:
            break # ret가 t/f

        frameCount += 1 # CPU GPU에 부담가지 않게
        if frameCount % 6 == 0: # 8은 화면 속도 조절
        ...
        ########################################################################################
            ...
            for i in np.arange(0, detections.shape[2]):
                ...
                if confidence > CONF_VALUE: # 신뢰값 정의
                    ...
                    # 사람들 percentlist
                    if idx == 15:
                        numOfPeopleProbList.append(confidence*100)
                    # print("[INFO] {}".format(label))
                    cv2.rectangle(image, (startX, startY), (endX, endY),
                        COLORS[idx], 2)
                    y = startY - 15 if startY - 15 > 15 else startY + 15
                    ...
                    if idx == 15:
                        ...
                        numOfSthElse += 1
                    print("사람수 :"+str(numOfPeople) +" 다른것들 수: "+str(numOfSthElse)+" 사람들 퍼센트: "+str(numOfPeopleProbList))
                    if numOfPeople > _numOfPeople:
                        _numOfPeople = numOfPeople
                        _numOfPeopleProbList = numOfPeopleProbList.copy()
                        _numOfSthElse = numOfSthElse

            frame = image
            # frame = equalizeIm(frame)
            #########################################
            ...
            if c == 27: # ESC키
                ...
                captureVideo()
                window.update()
                wait = cv2.waitKey() # 같이 멈추기

    print("최종: 사람수 :"+str(_numOfPeople) +" 다른것들 수: "+str(_numOfSthElse)+" 사람들 퍼센트: "+str(_numOfPeopleProbList))
...
# frame 평활화 함수
def  equalizeIm(image) :
    ## 중요! 코드. 출력영상 크기 결정 ##
    iH, iW = image.shape[0], image.shape[1]
    oH, oW = iH, iW
    ###### 메모리 할당 ################
    returnImage = malloc(oH, oW, layers=3)
    ####### 진짜 컴퓨터 비전 알고리즘 #####
    # 히스토그램
    histo = np.array([np.bincount(image[:,:,RGB].flatten(), minlength=256) for RGB in range(3)]) # 256을 min으로 잡아줘야 함
    sumHisto = np.array([[0] * 256 for _ in range(3)]); normalHisto = np.array([[0] * 256 for _ in range(3)])
    ...
    ## 정규화 누적 히스토그램
    sumHisto.astype(np.float16)
    normalHisto = (sumHisto / (iH*iW)) * 255
    normalHisto = normalHisto.astype(np.uint8)
    ## 영상처리
    for RGB in range(3):
        returnImage[:,:,RGB] = normalHisto[RGB][image[:,:,RGB]]
    # outImage[:, :, RGB] = normalHisto[RGB][inImage[:, :, RGB]]
...
def captureVideo():
    global window, canvas, paper, filename, inImage, outImage, inH, inW, outH, outW
    global photo, cvPhoto, frame # frame은 Video에서 온 객체
...
openCVMenu.add_command(label="사물 인식(딥러닝)-동영상 (평활화)", command=deep3OpenCV)
