...
면접에 가장 중요한 것  : 처음 대처하는 것!

어떻게 왔냐? : 기본적인 질문? : 너-무 주저리주저리 다 얘기는 또 별로! : 면접이란 일단 '대화'

겉으로 보이는 도드라짐이 오히려 더 눈에 띄인다! : 1분 PR 주저리 주저리는 별로다?

비언어적인게 더 크다!

내가 편안한 말을 하는게 좋다 : 웃으면서 얘기를 하는 게 더 좋다!

스토리텔링?

결국은 1분 안에 내 스토리를 짧게 잘 얘기해 주는 게 좋다!

모든걸 다 보여줘야 한다는 강박을 버려야함! : 오히려 너무 큰 기대를 주게된다?

성장 과정에 대한 축소판? => 비교적 최근의 사례 / 직무의 도움이 되는

​	저는 지금까지 어떻게 열정으로 제 커리어를 쌓아왔습니다. ~~~~

1분 자기소개는 : 그냥 길지 않게 해라~

면접에서의 하나의 '틀'

내 필드로 끌어들이기? / 필드로 끌어들인 후 필살기로 마무리!

질문을 끌어내는 PR이란 무엇일까?

경험 + 역량을 잘 설명하는 PR : 경험 + 역량 + 성공케이스

외운티 ㄴㄴ : 완벽 외우기 : 자연스럽게

필살기 : 유사경험 / 성공경험 / 인사이트

5가지의 필살기 중 2가지 1분 자기소개에 담기 <= 자기소개서에 담으면 가장 좋음

완벽히 암기

**1분 PR**

안녕하세요, 저는 개발하고 공부할 때 즐거운 예비사원 강태형입니다. 저는 이 자리에 오기 위해 다양한 방식으로 제 역량을 쌓아왔습니다.

첫번째, 공모전입니다. 빅 콘테스트와 앨리스 데이터 챌린지에 참여하며 실전 데이터 분석 능력을 기르고, AI 기술을 실제로 적용해 보았습니다.

두번째, 프로젝트입니다. 관련 교육과정을 수강하며 프로그래밍과 AI 기술을 적용한 프로젝트를 수행하였습니다.

세번째, 스터디입니다. AI 논문 스터디, spark, kaggle 등 스터디에 참여하며 제 역량의 범위를 넓혔습니다.

KT라는 대한민국 최고 기업에서 일하며 회사에 발전에 기여하고, 저 또한 발전하는 기회가 되었으면 합니다. 감사합니다!

마지막말 : 주인정신 : 회사에서 그냥 자리지키다가 월급받아가는 사람이 아니라, 상사입장에서 '든든한' 후임이 되고싶다!

**지원 동기**

이제는 세상이 너무 복잡해져서 가치 판단하기도 힘들어졌습니다. 하지만 그와 더불어 엄청난 양의 데이터가 생산되고, 이를 다룰 수 있는 기술이 생겼습니다.

이에 저는 이러한 데이터로부터 새로운 인사이트를 얻고자 하며, 이를 위해서는 AI 프로그래밍이 도구로 사용된다고 생각합니다.

그 공부들을 지금까지 열심히 해왔습니다. 이제는 이곳, KT에서 제 역량을 나타내며 더욱 가치있는 일을 해나가고 싶습니다. 꼭 일하고 싶습니다. 감사합니다!
...
뽑아야 할 사람을 찾는 것! : 공격형으로! => 나만의 강점을 잘 녹여내어 얘기

적당한 기업과의 밀당? 하핫

면접관의 진짜 공격 : 꼬리 질문!

너무 승부에 연연하지 마라 : 이것은 '평가전'
...
- before & after의 차이가 명확한 것 : 내가 무언가를 한 이후로!
- 수치의 변화
- 무 => 유
- 경쟁우위에 있던 것을 입증 : 입상 / 수상

그리고 '인사이트'

- 위 두가지를 멋지게 포장하는 방법 : 위 두가지에 대한 생각 정리
- 성공의 원리
- 실패의 교훈
- 구루의 인사이트 : 그 분야의 대가들이 하라고 했던 인사이트?
...
밉살기 : 성공경험이긴 한데 크기가 좀 작은 것



### 객체 지향 특징

기존의 만들었던 내용을 재사용성을 높이자!

1. 추상화 : class를 만드는 것 : '틀' 을 만드는 것 : 변수와 메서드
2. 캡슐화 : 객체의 숨기고픈 내용은 외부에서 접근 못하게 만들기 : 유지보수 효율성 높이기
3. 상속 : 코드의 재사용성을 증대시키고, 생산성 증대
4. 다형성 : 다양한 형태로 표현 / 오버로딩(하나의 함수에 여러 매개변수 가능), 오버라이딩(상속받은 함수 새로 만들어서 사용) : 코드 재사용성 증가

 

### 중간 정리 : elice data challenge
...
- 최고 어려웠던 도전 : 빅 콘테스트
  ...
  - 이 실패에서 얻은 교훈
    - 내 작업에만 너무 집중하다보니 남이 이미 해놓은 것을 제대로 보지 못함
    - 지난해 대회도 비슷한 내용이었는데, 트리 기반 솔루션으로 해답을 냄.
    - 정형 데이터에는 트리 기반 솔루션이 좀더 잘 적용된다
...
1차 : 로우마다 컬럼값이 1개씩! => 원자값

2차 : 기본키의 부분적 종속을 없애기 => 기본키가 중복되서 나타낼 필요 없는 경우 테이블을 쪼개기

3차 : 컬럼들간에 종속관계를 없애기 => 테이블 나누기
