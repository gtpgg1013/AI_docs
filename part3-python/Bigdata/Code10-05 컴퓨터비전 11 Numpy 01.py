...
def loadImage(fname) :
    ...
    ## 입력영상 메모리 확보 ##
    inImage = np.fromfile(fname,dtype=np.uint8) # 이건 한줄로 읽은거 : reshape해야댐 # uint8 : 0~255
...
def loadCSV(fname) :
    ...
    with open(fname, 'r') as rFp:
        for row_list in rFp:
            row, col, value = list(map(int,row_list.strip().split(','))) # row_list 각각에 int함수 적용해주고 그 전체에 list함수적용
